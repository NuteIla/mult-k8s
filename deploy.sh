...
kubectl apply -f k8s
kubectl set image deployments/server-deployment server=hazelnut95/multi-server:$SHA
kubectl set image deployments/client-deployment client=hazelnut95/multi-client:$SHA
kubectl set image deployments/worker-deployment worker=hazelnut95/multi-worker:$SHA